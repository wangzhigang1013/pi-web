...
test("tail pagination retains kept messages when firstKeptEntryId is older than the tail slice", () => {
  // Regression test: when firstKeptEntryId lies outside the recent tail window (e.g. 80 entries, tail 50,
  // firstKeptEntryId at e20), buildSessionContext must not drop the 49 messages in the tail slice.
  const entries = linearChain(80);
  entries.push({
    id: "compact",
    parentId: "e79",
    type: "compaction",
    timestamp: new Date(200000).toISOString(),
    summary: "Compacted previous work",
    firstKeptEntryId: "e20",
    tokensBefore: 50000,
  });

  const page1 = buildSessionContext(entries, "compact", { tail: 50 });
  // The compaction summary is placed first, followed by all kept messages in the tail slice (e31..e79)
  assert.equal(page1.messages.length, 50);
  assert.equal(page1.entryIds.length, 50);
  assert.equal(page1.entryIds[0], "compact");
  assert.equal(page1.entryIds[1], "e31");
  assert.equal(page1.entryIds[page1.entryIds.length - 1], "e79");
  assert.equal(page1.oldestEntryId, "e31");

  // Paging upward with `before: e31` loads the preceding slice (e0..e30)
  const page2 = buildSessionContext(entries, page1.oldestEntryId, { tail: 50, excludeLeaf: true });
  assert.equal(page2.messages.length, 31);
  assert.equal(page2.entryIds[0], "e0");
  assert.equal(page2.entryIds[page2.entryIds.length - 1], "e30");
  assert.ok(page2.entryIds.every((id) => !page1.entryIds.includes(id)));
});
