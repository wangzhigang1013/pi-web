...
test("buildSessionTreeStructure separates hidden workspaces and protects current active workspace", () => {
  const mockSessions = [
    {
      id: "s1",
      path: "/sessions/s1.jsonl",
    ...
    },
    {
      ...
      modified: "2025-01-01T10:00:00Z",
      messageCount: 2,
      firstMessage: "Older App Two",
    },
    {
      id: "s3",
      path: "/sessions/s3.jsonl",
      cwd: "C:/projects/app-three",
      projectRoot: "C:/projects/app-three",
      projectKey: "app-three-key",
      modified: "2025-01-03T10:00:00Z",
      messageCount: 1,
      firstMessage: "Misc App Three",
    },
  ];
  ...
  const recentProjects = [
    { root: "C:/projects/app-one", key: "app-one-key" },
    { root: "C:/projects/app-two", key: "app-two-key" },
    { root: "C:/projects/app-three", key: "app-three-key" },
  ];

  // Case 1: app-two-key is hidden
  const tree1 = buildSessionTreeStructure({
    allSessions: mockSessions,
    recentProjects,
    pinnedWorkspaces: [],
    customFolders: [],
    ...
    runningSessionIds: new Set(),
    unreadSessionIds: new Set(),
    hiddenWorkspaceKeys: new Set(["app-two-key"]),
  });

  assert.equal(tree1.workspaceGroups.length, 2);
  assert.deepEqual(tree1.workspaceGroups.map((g) => g.key), ["app-three-key", "app-one-key"]);
  assert.equal(tree1.hiddenWorkspaceGroups.length, 1);
  assert.equal(tree1.hiddenWorkspaceGroups[0].key, "app-two-key");
  assert.equal(tree1.hiddenWorkspaceGroups[0].families.length, 1);

  // Case 2: Even if current active workspace (app-one-key) is in hiddenWorkspaceKeys, it is kept visible
  const tree2 = buildSessionTreeStructure({
    allSessions: mockSessions,
    recentProjects,
    pinnedWorkspaces: [],
    customFolders: [],
    ...
    runningSessionIds: new Set(),
    unreadSessionIds: new Set(),
    hiddenWorkspaceKeys: new Set(["app-one-key", "app-two-key"]),
  });

  assert.equal(tree2.workspaceGroups.length, 2);
  assert.ok(tree2.workspaceGroups.some((g) => g.key === "app-one-key" && g.isCurrent));
  assert.equal(tree2.hiddenWorkspaceGroups.length, 1);
  assert.equal(tree2.hiddenWorkspaceGroups[0].key, "app-two-key");
});
